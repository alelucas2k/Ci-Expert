...
begin
    `ifdef ICARUS
        $display("[TB] Icarus Simulation: Dumping VCD...");
        $dumpfile("counter32bit.vcd");
        $dumpvars(0, counter_overflow_tb); // O nome do module no seu código é counter_overflow_tb
    `else
        $fsdbDumpfile("counter32bit.fsdb");
        $fsdbDumpvars();
    `endif
end
